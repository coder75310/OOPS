...
class InvalidAmountException: public runtime_error{
    public:
    InvalidAmountException(const string &msg) : runtime_error(msg){};  // iss se kya hoga mera jo runtime_error wala jo msg h vo mera update ho jaye
...
class customer{
    ...
    void deposit(int amount){
        ...
        if(amount<=0)
            throw InvalidAmountException("Insufficient amount");
    ...
    void withdraw(int amount){
        if(amount > 0 && amount<=balance){
            ...
            throw InvalidAmountException(" amount should be greater than 0");   // exception statement 
        }
        else{
